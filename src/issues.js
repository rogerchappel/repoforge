...
export async function writeInitialIssuesFile(options = {}) {
  const plan = await planInitialIssues({
    issuePlan: true,
    dryRun: true,
  ...
  const outputPath = path.join(options.targetDir, ".github", "repoforge-initial-issues.md");
  const content = renderIssuePlanMarkdown(plan);
...
function renderIssuePlanMarkdown(plan) {
  const lines = [
    "# Initial Issue Plan",
    "",
    "These issues were generated by repoforge. Review them before creating GitHub issues.",
    ""
  ];

  for (const issue of plan.issues) {
    lines.push(`## ${issue.title}`, "", issue.body.trimEnd(), "");
    const command = buildIssueCreateCommand(issue).shellCommand;
